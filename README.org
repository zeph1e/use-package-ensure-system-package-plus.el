...
so =sudo= only prompts for a password once per batch — with a live, color-coded
queue buffer and installer log so you can see exactly what is happening.

[[file:screenshots/overview.png]]

/Left: the queue buffer, one row per command, colored by status. Right: the
installer buffer, with the full transcript of every command run this
session./
...
1. Enqueues every install command into a FIFO queue, and gives it a row in
   the queue buffer, =*upesp+ queue*=.
...
4. Handles =sudo= password prompts without blocking Emacs, and lets =C-g= cancel
   a prompt cleanly (see [[*Cancelling and Restarting Installs][below]]).
5. Automatically bootstraps missing package managers (e.g. installs =npm= via
   =apt= before running an =npm install=).
6. Logs every command's output to the installer buffer, =*upesp+ installer*=,
   without erasing what came before — the whole session's transcript stays
   available, one command's section at a time, separated by a divider line.
7. When the queue drains, schedules the shell to close after an idle timeout
...
| =cancelled=    | Interrupted by you (see below)         | plain            |
...
Sometimes you want to stop a command that is already running — a =sudo=
password prompt you did not expect, or an install you know is going to
fail. Two ways to do that:
...
argument.  Use it to react to specific installations — for example, to
...
additional commands or call ~recursive-edit~ inside a hook function. The
hook does not receive the command's status; read the ~status~ field of its
entry in ~upesp+:queue-entries~ if you need that.
