...
4. Handles =sudo= password prompts without blocking Emacs. =C-g= cancels a
   prompt cleanly. See [[*Cancelling and Restarting Installs][Cancelling and Restarting Installs]].
5. Bootstraps a missing package manager automatically. For example, it
   installs =npm= through =apt= before it runs an =npm install=.
